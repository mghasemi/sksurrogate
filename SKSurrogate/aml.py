...
class StackingEstimator(BaseEstimator, TransformerMixin):
    ...
    :param cv: cross-validation splitter used to create out-of-fold training features; default=5
    :param n_jobs: number of parallel jobs used for out-of-fold predictions; default=None
    """

    def __init__(self, estimator, res=True, probs=True, decision=True, cv=5, n_jobs=None):
    ...
    def fit_transform(self, X, y=None, **fit_params):
        """Fit the estimator and return leakage-safe out-of-fold features.

        During pipeline training, predictions are generated by cloned estimators
        on folds that did not contain each sample. The wrapped estimator is then
        fitted on all training data for use by :meth:`transform` on later data.
        """
        import numpy as np
        ...
        predictions = {}
        if self.probs and hasattr(self.estimator, "predict_proba"):
            predictions["probs"] = cross_val_predict(
        ...
        if self.decision and hasattr(self.estimator, "decision_function"):
            predictions["decision"] = cross_val_predict(
                clone(self.estimator),
                X,
                y,
                cv=self.cv,
        ...
        if self.res:
            predictions["res"] = cross_val_predict(
                clone(self.estimator),
                X,
                y,
                cv=self.cv,
        ...
        if self.probs and "probs" in predictions:
            features.append(predictions["probs"])
        if self.decision and "decision" in predictions:
            decision = np.asarray(predictions["decision"])
            features.append(np.reshape(decision, (-1, 1)) if decision.ndim == 1 else decision)
        if self.res:
            result = np.asarray(predictions["res"])
            features.append(np.reshape(result, (-1, 1)))
        return np.hstack(features)
...
class AML(object):
    ...
    :param cv: default=3; The cross-validation method. Integer values use
        stratified folds for classification and regular folds for regression.
    ...
    def predict(self, X):
        """Predict targets with the best pipeline found by ``fit`` or ``eoa_fit``."""
        return self._require_fitted().predict(X)

    def predict_proba(self, X):
        """Predict class probabilities with the best classifier pipeline."""
        estimator = self._require_fitted()
        if not hasattr(estimator, "predict_proba"):
            raise AttributeError("The selected estimator does not provide predict_proba")
        return estimator.predict_proba(X)

    def score(self, X, y):
        """Return the best pipeline's estimator score on ``X`` and ``y``."""
        return self._require_fitted().score(X, y)

    def get_params(self, deep=True):
        """Return constructor parameters using the scikit-learn convention."""
        return {
            "config": self.config,
            "length": self.length,
            ...
            "min_random_evals": self.min_random_evals,
            "cv": self.cv,
            "check_point": self.check_point,
    ...
    def set_params(self, **params):
        """Set constructor parameters and return this AML instance."""
            ...
            if (
                ...
                while (
                    ...
                    if next_est in self.known_feature_selectors:
                        int_mdl = self._new_estimator(self._get_class(next_est))
