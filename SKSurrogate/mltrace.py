...
class mltrack(object):
    ...
    def LogModel(self, mdl, name=None):
        ...
        if 'mltrack_id' not in mdl.__dict__:
            MLModel.create(task_id=self.task_id, name=mdl.mltrack_name, model_str=str(mdl),
                           model_type=str(type(mdl)).split("'")[1],
                           parameters=dumps(mdl.get_params()))
            mdl.mltrack_id = MLModel.select(MLModel.model_id).order_by(MLModel.model_id.desc()).get()
        else:
    ...
    def LogMetrics(self, mdl, cv=None):
        ...
        if mdl_type == 'classifier':
            ...
            for i in range(int(n_)):
                fpr, tpr, auc_thresholds = roc_curve(prds[i][1], prbs[i])
                aur += auc(fpr, tpr)
            aur /= n_
            ...
            vrn = sum([explained_variance_score(y_tst, y_prd) for y_prd, y_tst in prds]) / n_
            mxe = sum([median_absolute_error(y_tst, y_prd) for y_prd, y_tst in prds]) / n_
        ...
        res.last_mod_date = datetime.now()
        res.save()
        return dict(accuracy=acc, auc=aur, precision=prs, f1=f_1, recall=rcl, mcc=mcc, logloss=lgl, variance=vrn,
                    max_error=mxe, mse=mse, mae=mae, r2=r2)
    ...
    def LoadModel(self, id):
        ...
        res = MLModel.select().where(MLModel.model_id == id)
        if len(res) == 0:
            raise BaseException("No model with id '%d' were found" % (id))
    ...
    def getBest(self, metric):
        ...
        """
        res = Metrics.select().order_by(Metrics.__dict__[metric].__dict__['field'].desc()).dicts()
        return res[0]
    ...
    def allPreserved(self):
        ...
        from pandas import read_sql
        return read_sql(
            "SELECT pickle_id, model_id, init_date FROM saved", self.conn)
    ...
    def RecoverModel(self, mdl_id):
        ...
        """
        from sklearn.externals import joblib
        res = Saved.select().where(Saved.model_id == mdl_id).order_by(Saved.init_date.desc()).dicts()
        file = open('track_ml_tmp_mdl.joblib', 'wb')
    ...
    def allPlots(self, mdl_id):
        ...
        from pandas import read_sql
        return read_sql(
            "SELECT plot_id, model_id, title, init_date FROM plots WHERE model_id=%d" % (mdl_id), self.conn)
    ...
    def LoadPlot(self, id):
        ...
        from pickle import loads
        # ax = plt.subplot(111)
        res = Plots.select().where(Plots.plot_id == id).dicts()
        fig = loads(res[0]['plot'])
        return fig

    def plot_learning_curve(self, mdl, title, ylim=None, cv=None,
                            n_jobs=1, train_sizes=None, **kwargs):
        """
        ...
        train_sizes, train_scores, test_scores = learning_curve(
            mdl, self.X, self.y, cv=self.cv, n_jobs=n_jobs, train_sizes=train_sizes, scoring=meas)
        xlbls = np.array([str(round(_ * 100, 1)) + " %" for _ in train_sizes / len(self.y)])
        ...
        plt.fill_between(xlbls, train_scores_mean - train_scores_std,
                         train_scores_mean + train_scores_std, alpha=0.1,
                         color="r")
        plt.fill_between(xlbls, test_scores_mean - test_scores_std,
                         test_scores_mean + test_scores_std, alpha=0.1, color="g")
        plt.plot(xlbls, train_scores_mean, 'o-', color="r",
                 label="Training score")
        plt.plot(xlbls, test_scores_mean, 'o-', color="g",
    ...
    def plot_calibration_curve(self, mdl, name, fig_index=1, bins=10):
        ...
        X_train, X_test, y_train, y_test = train_test_split(self.X, self.y, train_size=.75)
    ...
    def plot_cumulative_gain(self, mdl, title='Cumulative Gains Curve',
        ...
        percentages, gains1 = self.cumulative_gain_curve(y_true, prob_pos0,
                                                         classes[0])
        percentages, gains2 = self.cumulative_gain_curve(y_true, prob_pos1,
                                                         classes[1])
    ...
    def cumulative_gain_curve(self, y_true, y_score, pos_label=None):
        ...
        """
        from numpy import asarray, array_equal, sum, cumsum, arange, insert, unique, argsort
    ...
    def plot_lift_curve(self, mdl, title='Lift Curve',
                        figsize=None, title_fontsize="large",
                        text_fontsize="medium"):
        """
    ...
    def heatmap(self, corr_df=None, sort_by=None, ascending=False, font_size=3, cmap="gnuplot2", idx_col='feature',
        ...
        ax.autoscale(False)
        cbar_kw = {'fraction': .2, 'ticks': [mn, 0., (mn + mx) / 2., mx], 'drawedges': False}
        ...
        plt.setp(ax.get_xticklabels(), rotation=-305, ha="left", va='top', rotation_mode="anchor")
    ...
    def FeatureWeights(self, weights=('pearson', 'variance'), **kwargs):
        ...
        for factor in weights:
            if factor == 'pearson':
                ...
                RF.fit(X, y)
                W['relieff'] = [RF.feature_importances_[features.index(v)] for v in features]
            elif factor == 'surf':
                from skrebate import SURF
                RF = SURF(n_features_to_select=n_features)
                RF.fit(X, y)
                W['surf'] = [RF.feature_importances_[features.index(v)] for v in features]
            elif factor == 'sobol':
                from .sensapprx import SensAprx
                SF = SensAprx(method='sobol', domain=domain, probs=probs, regressor=regressor, reduce=reduce,
                              num_smpl=num_smpl)
                SF.fit(X, y)
                domain = SF.domain
                probs = SF.probs
                ...
                from .sensapprx import SensAprx
                SF = SensAprx(method='morris', domain=domain, probs=probs, regressor=regressor, reduce=reduce,
                              num_smpl=num_smpl)
                SF.fit(X, y)
                domain = SF.domain
                probs = SF.probs
                ...
                from .sensapprx import SensAprx
                SF = SensAprx(method='delta-mmnt', domain=domain, probs=probs, regressor=regressor, reduce=reduce,
                              num_smpl=num_smpl)
                SF.fit(X, y)
                domain = SF.domain
                probs = SF.probs
